...
// FIX: isPremium now checks premium_until with 3-day grace period
// This is used for feature gating only (respects PREMIUM_ENABLED flag)
function isPremium(user) {
  if (!PREMIUM_ENABLED) return true;
  if (!user.is_premium) return false;
  if (!user.premium_until) return true; // no expiry set, treat as active
  const gracePeriodMs = 3 * 24 * 60 * 60 * 1000;
  return new Date(user.premium_until).getTime() + gracePeriodMs > Date.now();
}

// Separate check: has user ever paid for premium (regardless of PREMIUM_ENABLED)
// Used for the upgrade command to show correct status message
...
app.get("/cron/check-premium", async (req, res) => {
  ...
  // FIX: fetch only users whose premium hasn't expired yet (efficient query)
  ...
  for (const u of premiumUsers) {
    ...
    // FIX: grace period is exactly 3 days after expiry
    const graceEnd = new Date(expiryDate.getTime() + 3 * 24 * 60 * 60 * 1000);

    // Downgrade — grace period has passed
    if (now > graceEnd) {
      await supabase
        .from("users")
        .update({
...
We hope to see you back! 🙏`
    ...
    // 3-day warning — FIX: only send once using premium_warned_3d flag
    if (
...
Questions? contact@carlogbook.app`
    ...
    // 1-day warning — FIX: only send once using premium_warned_1d flag
    if (
...
Example: T123ABC`;
  ...
  // ── CANCEL COMMAND (moved early — must run before any flow checks) ─────────
  ...
  if (from === ADMIN_PHONE) {
    ...
    if (text.toLowerCase().startsWith("approve ")) {
      ...
      // FIX: reset warning flags on renewal so user gets warnings next cycle
      await supabase
        .from("users")
        .update({
          is_premium: true,
...
Tip: If you have multiple cars, include the car name in your message so I know which one to log against.`;
  ...
  if (text.toLowerCase() === "upgrade") {
    ...
    // FIX: use isActivePremiumUser() — works regardless of PREMIUM_ENABLED flag
...
Questions? Contact us at contact@carlogbook.app`;
    ...
    // check if transaction ID already exists (duplicate)
